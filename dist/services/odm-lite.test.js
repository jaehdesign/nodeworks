...
describe('Given a instance of class ORMLite', () => {
    ...
    beforeEach(() => {
        ...
        vi.mocked(readFromDisk).mockResolvedValue(JSON.stringify({
            items: ITEMS,
        }));
    ...
    describe('When run method readById with a collection name and an id', () => {
        ...
        test('Then a error should be throw if id is 3', async () => {
            const collection = 'items';
            const id = '3';
            await expect(odmLite.readById(collection, id)).rejects.toThrowError(`Item with id ${id} not found`);
        });
    });
    describe('When run method create with a collection name and data', () => {
        test('Then result should be the item created', async () => {
            ...
            expect(writeToDisk).toHaveBeenCalledWith(expect.any(String), expect.stringContaining(initialData.title));
        });
    });
    describe('When run method updateById with a collection name, id and data', () => {
        test('Then result should be the item updated if id is 1', async () => {
            ...
            expect(writeToDisk).toHaveBeenLastCalledWith(expect.any(String), expect.stringContaining(data.title));
        });
        test('Then a error should be throw if id is 3', async () => {
            const collection = 'items';
            const id = '3';
            const data = { title: 'Item 3 Updated' };
            await expect(odmLite.updateById(collection, id, data)).rejects.toThrowError(`Item with id ${id} not found`);
        });
    });
    describe('When run method deleteById with a collection name and id', () => {
        test('Then result should be the item deleted if id is 1', async () => {
            ...
            expect(item).toStrictEqual(DB.items[0]);
            expect(writeToDisk).toHaveBeenCalledWith(expect.any(String), expect.not.stringContaining(DB.items[0].title));
        });
        test('Then a error should be throw if id is 3', async () => {
            const collection = 'items';
            const id = '3';
            await expect(odmLite.deleteById(collection, id)).rejects.toThrowError(`Item with id ${id} not found`);
        });
    });
});
